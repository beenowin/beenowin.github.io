...
// EXTERNAL MODULE: ./src/services/index.js + 7 modules
var services = __webpack_require__(9443);
...
const Home = () => Promise.all(/* import() */[__webpack_require__.e(736), __webpack_require__.e(64), __webpack_require__.e(198)]).then(__webpack_require__.bind(__webpack_require__, 9761));
const Post = () => Promise.all(/* import() */[__webpack_require__.e(736), __webpack_require__.e(64), __webpack_require__.e(261)]).then(__webpack_require__.bind(__webpack_require__, 4394));
...
const routes = [{
  path: "/",
  name: "Home",
  component: () => Promise.all(/* import() */[__webpack_require__.e(736), __webpack_require__.e(64), __webpack_require__.e(73)]).then(__webpack_require__.bind(__webpack_require__, 1073)),
  children: [{
    path: "",
    component: Home
  }]
  ...
  children: [{
    path: "",
    component: () => Promise.all(/* import() */[__webpack_require__.e(736), __webpack_require__.e(64), __webpack_require__.e(337)]).then(__webpack_require__.bind(__webpack_require__, 337))
  }]
}, {
  path: "/:pathMatch(.*)*",
  name: "NotFound",
...
// EXTERNAL MODULE: ./node_modules/quasar/src/plugins/Notify.js + 4 modules
var Notify = __webpack_require__(4877);
...
/* harmony default export */ const en = ({
  ...
  project_description: "<b>Beenow</b> is an aggregator of the Ukrainian (and not only) Internet news 🇺🇦 launched in 2022. Our service collects materials — news and everything with them related — from mass media and author's channels in telegrams and combines into a news feed. The service is based on software code, that is, the information strip is formed automatically, and not by hand editors Links placed in the news feed should not be considered as an endorsement by the author of the Beenow internet portal of the views that expressed on the relevant Internet pages. Some links can be promotional. There may also be news of a satirical nature, which are not is real news. Therefore, do not trust everything that can happen written Check the information in the sources. Think with your head 🧠 and do not be manipulated. Enjoy using the Beenow service! 😉",
  for_contact: "For communication and contact with the author of the project write to this email address:",
  or: "or in",
  telegram: "telegram"
});
...
/* harmony default export */ const uk = ({
  ...
  "project_description": "<b>Beenow</b> – це агрегатор новин українського(і не тільки) інтернету 🇺🇦  запущений у 2022 році. Наш сервіс збирає матеріали — новини і все з ними пов'язане — із засобів масової інформації та авторських каналів у телеграмі та об'єднує в новинну стрічку. Сервіс базується на програмному коді, тобто, інформаційна стрічка формується автоматично, а не руками  редакторів. Посилання, що розміщуються у стрічці новин, не слід розглядати як схвалення автором інтернет-порталу Beenow поглядів, що  висловлюються на відповідних інтернет-сторінках. Деякі посилання можуть бути рекламними. Також можуть бути новини сатиричного характеру, які не є справжніми новинами. Тому не довіряйте усьому тому що може бути написано. Перевіряйте інформацію у джерелах. Думайте своєю головою 🧠 і не піддавайтеся на маніпуляції. Приємного користування сервісом Beenow! 😉",
  "for_contact": "Для зв'язку та контакту з автором проекту пишить на цю електронну адресу:",
  "or": "або в",
  "telegram": "телеграмі"
});
...
class _ChannelService {
  constructor() {
    ...
    this.loadChannelsCategories = async () => {
      this.categories = await this.getChannelsCategories();
    };
    ...
    this.loadChannels = async () => {
      ...
      this.storageChannels = JSON.parse(localStorage.getItem('channels') || null);
      ...
      if (!this.storageChannels) {
        Object.keys(this.channels).forEach(key => {
          ...
          this.channels[key].channelNickname = channel.telegramName || channel.channelNickname;
        });
        ...
        Object.keys(this.storageChannels).forEach(key => {
          if (this.channels[key]) {
            this.channels[key] = {
              ...this.channels[key],
              active: this.storageChannels[key]?.hasOwnProperty('active') ? this.storageChannels[key].active : this.channels[key].active
            };
          } else {
    ...
    this.updateChannel = (nickname, data) => {
      const channels = JSON.parse(localStorage.getItem('channels'));
      channels[nickname] = {
        ...this.channels[nickname],
        ...data
      };
    ...
    this.getStorageChannels = () => {
      this.storageChannels = localStorage.getItem('channels') || null;
      if (this.storageChannels) {
        this.storageChannels = JSON.parse(this.storageChannels);
      }
      return this.storageChannels || {};
    };
    RequestPreloadQueue.addRequest(this.loadChannelsCategories);
    RequestPreloadQueue.addRequest(this.loadChannels);
  }
}
...
let page = 1;
let totalPages = 0;
const getChannels = () => Object.keys(ChannelService.getStorageChannels()).filter(key => {
  if (ChannelService.getStorageChannels()[key].active) {
    return localStorage.getItem('category') !== 'all' ? ChannelService.getStorageChannels()[key].category === localStorage.getItem('category') : true;
  }
  return false;
});
const fetchPosts = async page => axios_default().get(`${constants/* VUE_APP_API_HOST */.M}/api/posts`, {
  params: {
...
/* harmony default export */ const post = ({
  ...
  actions: {
    ...
    async loadMorePosts({
      ...
      try {
        const posts = await service.loadMorePosts();
        commit('addPosts', posts);
      } catch (error) {
        console.log(error.message);
      }
...
/******/ 			return "js/" + (chunkId === 64 ? "chunk-common" : chunkId) + "." + {"64":"cdd32121","73":"1e25fd2e","198":"19016942","261":"a119dbe1","337":"2f8ee8e0","771":"57047214","857":"614a45df"}[chunkId] + ".js";
/******/ 		};
/******/ 	})();
/******/ 	
...
/******/ 			// return url for filenames based on template
/******/ 			return "css/" + chunkId + "." + {"73":"c3e8b107","198":"f4adbe21","261":"7663dfa0","337":"80f35630","771":"f600840c","857":"e2777194"}[chunkId] + ".css";
/******/ 		};
/******/ 	})();
/******/ 	
...
/******/ 			var cssChunks = {"73":1,"198":1,"261":1,"337":1,"771":1,"857":1};
...
/******/ })()
;
//# sourceMappingURL=app.a4825299.js.map